...
test.describe('Login does not flash logged-out UI before notice', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // /auth/v returns the authenticated user once login has succeeded.
    await page.route(
    ...
    await page.route(
      ...
      (route) => route.fulfill({
        ...
        body: JSON.stringify({
          type: 'doc',
          content: [
            { type: 'paragraph', content: [{ type: 'text', text: 'Test notice' }] },
          ],
        }),
      }),
    )
  ...
  test('logged-out header never paints between login and notice', async ({ page }) => {
    await openLoginModal(page)
    await page.fill('#login-username', 'testuser')
    await page.fill('#login-password', 'testpass123')
    ...
    expect(flashes, 'Login button re-appeared post-submit (logged-out repaint)').toBe(0)
  })
})
