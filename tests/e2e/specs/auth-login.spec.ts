...
test.describe('Login Modal', () => {
  ...
  test('opens login modal when clicking deposit while unauthenticated', async ({ page }) => {
  ...
  test('password visibility toggle works', async ({ page }) => {
    ...
    const toggleBtn = page.locator('#login-password').locator('..').locator('button').first()
  ...
  test('close button has aria-label', async ({ page }) => {
    await openLoginModal(page)
    const closeBtn = page.locator('.login-modal-wrapper').locator('..').locator('button[aria-label]').first()
  ...
  test('closes modal on close button click', async ({ page }) => {
    ...
    if ((await closeBtn.count()) > 0) {
      ...
      await expect(page.locator('#login-username')).not.toBeVisible({ timeout: 3_000 })
    }
  })

  test('successful login closes modal and updates UI', async ({ page }) => {
    // Re-route auth/v to return authenticated user after login
    await page.route(
      (url) => url.toString().includes('/auth/v'),
      (route) => route.fulfill({
        status: 200,
  ...
  test('shows error on invalid credentials', async ({ page }) => {
    ...
    const errorIndicator = page.locator('[data-sonner-toast], .text-red-500, .swal2-popup').first()
  ...
  test('switch to signup modal', async ({ page }) => {
    ...
    if ((await signUpLink.count()) > 0) {
      ...
      await expect(page.locator('#login-username')).not.toBeVisible({ timeout: 5_000 })
    }
  })
})
