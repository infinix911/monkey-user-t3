...
- **API layer:** `app/composables/useApi.ts` (isomorphic `$fetch`; SSR → `API_HOST_URL`+cookie, client → `/api` proxy; CSRF echo; 401 auto-logout; optional `.validated<T>(zodSchema)`). Store mutations use `app/lib/axios-client.ts` (parity: CSRF + 401 latch). **Reuse both — do not add new clients.**
- **Proxy:** `server/routes/api/[...path].ts` forwards `/api/*` → `API_HOST_URL`.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------ |
| `/auth/sign-in/username`,`/register`,`/change-password`,`/logout`,`/v`,`/ws`,`/referrals`,`/check/username`,`/check/referral`,`/register-offline-telegram` | ✅      | ⚠️ casing + field/length mismatches                                                  |
...
- **Backend `/auth/get-session`:** `{ id, upperId, depth, username, bankName, bankAccount, bankAccountName, phone, userType, wallet:string, pointWallet:string, ... }` or `null` — **camelCase, no `currency`**.
...
- **Frontend now:** login/register components + `authStore` (axios-client for `/auth/get-session`,`/logout`); snake_case mapping; signup has `currency`+`mobile` fields; login username max 32.
- **Backend:** Better Auth `POST /auth/sign-in/username {username,password}`; `POST /auth/register {username(5-12),email?,password(6-20),confirmPassword(4-20),phone(8-15),bankName,bankAccount,bankAccountName,referral?}`; `GET /auth/get-session` (camelCase member profile or `null`, see F2); `POST /change-password`, `/logout`, `GET /ws`, `/referrals`, `POST /check/username`,`/check/referral`.
...
- **Response mapping:** `/auth/get-session` per F2.
...
- **Auth:** 🔒 endpoints need `bn.session`; `useApi`/`axios-client` already send cookies + CSRF + 401 latch — **reuse, don't reinvent**. Anonymous `GET /auth/get-session` returns `200 null` and is handled by `authStore.verifyUser()`.
...
6. 🟠 **`/auth/get-session` has no `currency`** — derive it from site currency, not the profile.
...
**Phase 1 — Auth core (M1):** login, register (drop `/banks/register`, map fields), `/auth/get-session`, logout, change-password, checks. Unblocks all 🔒 calls.
