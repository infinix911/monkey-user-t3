    ...
    <div class="w-full px-3 md:px-5 pt-5 pb-4 md:py-6 max-w-md mx-auto">
      <form @submit.prevent="onSubmit">
        ...
        <div class="mt-4">
          ...
          <!-- Reveal toggle, as on the login/signup fields. `pr-12` keeps the
               text clear of the button; unlike LoginModal's field the padding
               here is a utility, not an inline style, so it is not overridden. -->
          <div class="relative">
            <input v-model="passwordField" :type="revealPassword ? 'text' : 'password'"
              autocomplete="current-password" maxlength="100" :placeholder="t('withdrawal.passwordPlaceholder')"
              class="px-4 pr-12 py-3 rounded w-full text-base lg:text-[20px] h-[53px]" :style="{
                backgroundColor: dep.inputBgColor,
                color: dep.inputTextColor,
                border: `1px solid ${dep.inputBorderColor}`,
                borderRadius: '4px',
            ...
            <button type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-white/50 hover:text-white/90 transition-colors cursor-pointer"
              :aria-label="revealPassword ? t('password.hide') : t('password.show')"
              @click="revealPassword = !revealPassword">
              <!-- Eye-off (currently revealed → click to hide) -->
              <svg v-if="revealPassword" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
...
// Reveal toggle for the withdrawal password. Local and per-instance; reset
// after a successful submit so the next request never opens pre-revealed.
