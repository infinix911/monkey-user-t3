          ...
          <div class="mt-2 md:mt-6 pl-3 lg:pl-auto pr-3 md:pr-0">
            <div class="w-full mx-auto mt-4">
              ...
              <div class="mb-6">
                ...
                >
                  {{ t("partner.partnerWithdraw") }}
                </h2>
                <p class="text-[#AFAFAF] text-[14px] mt-2">
                  {{ t("partner.partnerWithdrawDesc") }}
                </p>
              </div>
              ...
              <div class="mb-5">
                ...
                <div class="grid grid-cols-4 gap-2 lg:gap-3 mt-6">
                  ...
                  <div class="relative">
                    ...
                    <button
                      type="button"
                      class="text-[18px] md:text-[20px] h-[45px] md:h-[50px] relative text-black transition-all text-center z-10 hover:scale-105 active:scale-95 font-medium"
                      :class="{ 'ring-2 ring-[#ffe100]': lastSelectedButton === 'MAX' }"
                      :style="{
                        backgroundColor: '#f599ff',
                        borderRadius: '12px',
                        fontWeight: 600,
                        border: lastSelectedButton === 'MAX' ? '1px solid #ffe100' : '1.7px solid #3C3C3C',
                        width: '100%',
                        color: '#000',
                        textAlign: 'center',
                        fontFamily: 'var(--font-line-seed)',
                      ...
                      @click="handleMax"
                      @mouseenter="($event.target as HTMLElement).style.backgroundColor = '#f6a8ff'"
                      @mouseleave="($event.target as HTMLElement).style.backgroundColor = '#f599ff'"
                    >
                      {{ t("withdrawal.max") }}
                    </button>
                  </div>
              ...
              <div class="relative mt-[40px] mb-[10px]">
                ...
                >
                  {{ isSubmitting ? t("withdrawal.submitting") : t("partner.requestWithdraw") }}
                </button>
              </div>

              <!-- My Request History -->
              <div class="mb-15">
                <PartnerMyHistory ref="historyRef" type="withdrawal" />
              </div>
            </div>
          </div>
        </form>
...
const schema = toTypedSchema(
  z.object({
    amount: z
      .string()
      .refine((v) => /^\d+$/.test(v.replace(/,/g, "")), () => ({ message: t("withdrawal.withdrawCheck") }))
      .refine((v) => Number(v.replace(/,/g, "")) > 0, () => ({ message: t("withdrawal.withdrawCheck") }))
      .refine((v) => Number(v.replace(/,/g, "")) >= 10000, () => ({ message: t("withdrawal.withdrawCheck") }))
      .refine((v) => Number(v.replace(/,/g, "")) % 1000 === 0, () => ({ message: t("withdrawal.withdrawDivCheck") })),
  })
);
...
function handleMax() {
  ...
  const divisibleValue = walletValue % 1000;
  amount.value = divisibleValue !== 0
    ? (walletValue - divisibleValue).toString()
...
const veeSubmit = veeHandleSubmit(async (values) => {
  ...
  try {
    ...
    await showSuccessAlert(
      t("partner.partnerWithdraw"),
      t("partner.apiMessages.PARTNER_WITHDRAWAL_REQUEST_SUCCESS")
    );
    ...
    await showErrorAlert(t("partner.partnerWithdraw"), translatedMessage);
  } finally {
